...
all_lens = (
    np.concatenate(all_lens_list) if all_lens_list else np.array([], dtype=np.int32)
)
all_entropy = (
    np.concatenate(all_entropy_list)
    if all_entropy_list
    else np.array([], dtype=np.float32)
)

match_entropy = (
    np.concatenate(match_entropy_list)
    if match_entropy_list
    else np.array([], dtype=np.float32)
)
match_distance = (
    np.concatenate(match_distance_list)
    if match_distance_list
    else np.array([], dtype=np.float32)
)
match_length = (
    np.concatenate(match_length_list)
    if match_length_list
    else np.array([], dtype=np.float32)
)
    ...
    children=[
        ...
        dcc.Tabs(
            ...
            children=[
                ...
                dcc.Tab(
                    label="2. Distance Distribution (1% Buckets)", value="distance-dist"
                ),
                dcc.Tab(
                    label="3. Length Distribution (1% Buckets)", value="length-dist"
                ),
                dcc.Tab(
                    label="4. Cumulative Length Contribution", value="length-contrib"
                ),
                dcc.Tab(
                    label="5. Entropy Profile (1% Timeline Chunks)",
                    value="entropy-timeline",
                ),
                dcc.Tab(
                    label="6. Cumulative Distance Contribution",
                    value="distance-contrib",
                ),
            ],
        ),
        html.Div(style={"marginTop": "20px"}, children=[dcc.Graph(id="tab-graph")]),
        html.Div(
            id="token-summary",
            style={
                "marginTop": "20px",
            ...
            children=[
                html.Strong("Total Literals (lit): "),
                f"{total_lits:,}",
                html.Br(),
                html.Strong("Total Matches (match): "),
                f"{total_matches:,}",
            ],
        ),
    ],
...
@app.callback(Output("tab-graph", "figure"), Input("analysis-tabs", "value"))
def update_graph(selected_tab):
    ...
    if selected_tab == "entropy-dist":
        ...
        fig.update_layout(
            ...
            xaxis=dict(
                title="File Progress (% Bytes Covered)",
                tickmode="linear",
                tick0=0,
                dtick=10,
            ),
