...
std::pair<std::byte *, size_t &> div_encoder::for_loading() {
    return {data.data(), bytes_loaded};
}

void div_encoder::process(auto future_limit, const auto NONE, auto i,
    ...
    for (size_t ii = 0; ii < workers; ii++) {
        pool.enqueue([&, ii]() {
            ...
            for (uint32_t pos = *ptr; pos + config::window_size >= i && --count > 0;
                 pos = *(ptr -= workers)) {
...
std::vector<token> div_encoder::encode() {
