...
std::vector<token> mpo_encoder::encode() {
    ...
    for (uint32_t i = 0; i < bytes_loaded; i++) {
        ...
        process(future_limit, NONE, i, best_match_len, best_match_pos);

        // constexpr float COST_MULT = 0.125f;
    ...
    while (i > 0) {
        ...
        // edge
        auto future_limit = std::min(bytes_loaded - came_from, config::future_limit);
        uint32_t best_match_len, best_match_pos;
        process(future_limit, NONE, came_from, best_match_len, best_match_pos);
        if (best_match_len == 1) {
            throw std::runtime_error(
                std::format("Optimal encoder has failed miserably at {} with "
                            "best_match_len={} and future_limit={}",
                            i, best_match_len, future_limit));
        }
