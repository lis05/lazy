...
struct header {
    ...
    friend std::ofstream& operator<<(std::ofstream& out, const header& h) {
        ...
        out.write(reinterpret_cast<const char*>(&h.bytes_dist),
                  sizeof(h.bytes_dist));
...
static std::vector<token> read_block(std::ifstream& in) {
    ...
    for (size_t i = 0; i < h.n_control; ++i) {
        if (control[i] == std::byte{0}) {
            ...
            tokens.push_back(match{.distance = dist[dist_idx++], .length = len[len_idx++]});
        }
    }
...
struct format {
    ...
    static format get_turbo2() {
        return format{turbo2::write_format_mark, turbo2::write_block,
                      turbo2::read_block, turbo2::verify_config};
    }
