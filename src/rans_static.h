// modified https://github.com/jkbonfield/rans_static/blob/master/r32x16b_avx2.c
/*
...
// Simple byte-aligned rANS encoder/decoder - public domain - Fabian 'ryg' Giesen
// 2014
//
...
static inline RansState RansEncRenorm(RansState x, uint8_t **pptr, uint32_t freq,
                                      uint32_t scale_bits) {
    uint32_t x_max =
        ((RANS_BYTE_L >> scale_bits) << 8) * freq - 1;  // this turns into a shift.
    if (x > x_max) {
        uint8_t *ptr = *pptr;
        do {
            *--ptr = (uint8_t)(x & 0xff);
...
static inline void RansEncSymbolInit(RansEncSymbol *s, uint32_t start, uint32_t freq,
                                     uint32_t scale_bits) {
...
static inline void RansEncPutSymbol(RansState *r, uint8_t **pptr,
    ...
    *r = x + sym->bias + q * sym->cmpl_freq;
    ...
    //    assert(((x / sym->freq) << sym->scale_bits) + (x % sym->freq) + sym->start
    //    == *r);
}
...
static inline void RansDecAdvanceStep(RansState *r, uint32_t start, uint32_t freq,
                                      uint32_t scale_bits) {
    uint32_t mask = (1u << scale_bits) - 1;
...
static inline void RansDecAdvanceSymbolStep(RansState *r, RansDecSymbol const *sym,
                                            uint32_t scale_bits) {
...
static void hist4p(unsigned char *in, unsigned int in_size, int *F0) {
    ...
    while (in < in_end) F0[*in++]++;

    for (i = 0; i < 256; i++) F0[i] += F1[i] + F2[i] + F3[i];
}
#endif

static void hist8(unsigned char *in, unsigned int in_size, int F0[256]) {
    int F1[256 + MAGIC] = {0}, F2[256 + MAGIC] = {0}, F3[256 + MAGIC] = {0};
    int F4[256 + MAGIC] = {0}, F5[256 + MAGIC] = {0}, F6[256 + MAGIC] = {0},
                 F7[256 + MAGIC] = {0};
    ...
    i *= 4;
    while (i < in_size) F0[in[i++]]++;

    for (i = 0; i < 256; i++)
        F0[i] += F1[i] + F2[i] + F3[i] + F4[i] + F5[i] + F6[i] + F7[i];
}

static unsigned int rans_compress_bound_32x16(unsigned int size, int order,
                                              int *tab) {
...
static __m256i _mm256_mulhi_epu32(__m256i a, __m256i b) {
    ...
    ab_hm =
        _mm256_and_si256(ab_hm, _mm256_set1_epi64x((uint64_t)0xffffffff00000000));
...
static inline __m256i _mm256_i32gather_epi32x(int *b, __m256i idx, int size) {
    ...
    return _mm256_set_epi32(b[c[7]], b[c[6]], b[c[5]], b[c[4]], b[c[3]], b[c[2]],
                            b[c[1]], b[c[0]]);
}
#else
...
static unsigned char *rans_compress_O0_32x16(unsigned char *in, unsigned int in_size,
                                             unsigned char *out,
    ...
    fsum++;  // not needed, but can't remove without removing assert x<TOTFREQ (in
             // old code)
    ...
    *cp++ = 0;
    ...
    for (z = 0; z < NX; z++) RansEncInit(&ransN[z]);

    z = i = in_size & (NX - 1);
    while (z-- > 0) RansEncPutSymbol(&ransN[z], &ptr, &syms[in[in_size - (i - z)]]);
...
#define SET1(a, b)                                                               \
    __m256i a##1 = _mm256_set_epi32(b[c[7]], b[c[6]], b[c[5]], b[c[4]], b[c[3]], \
                                    b[c[2]], b[c[1]], b[c[0]])
#define SET2(a, b)                                                          \
    __m256i a##2 = _mm256_set_epi32(b[c[15]], b[c[14]], b[c[13]], b[c[12]], \
                                    b[c[11]], b[c[10]], b[c[9]], b[c[8]])
#define SET3(a, b)                                                          \
    __m256i a##3 = _mm256_set_epi32(b[c[23]], b[c[22]], b[c[21]], b[c[20]], \
                                    b[c[19]], b[c[18]], b[c[17]], b[c[16]])
#define SET4(a, b)                                                          \
    __m256i a##4 = _mm256_set_epi32(b[c[31]], b[c[30]], b[c[29]], b[c[28]], \
                                    b[c[27]], b[c[26]], b[c[25]], b[c[24]])
#define SET(a, b) \
    SET1(a, b);   \
    SET2(a, b);   \
    SET3(a, b);   \
    SET4(a, b)
        ...
        __m256i Rs;
        Rs = _mm256_srli_epi32(Rv1, 16);
        Rv1 = _mm256_blendv_epi8(Rv1, Rs, cv1);
        Rs = _mm256_srli_epi32(Rv2, 16);
        Rv2 = _mm256_blendv_epi8(Rv2, Rs, cv2);
        Rs = _mm256_srli_epi32(Rv3, 16);
        Rv3 = _mm256_blendv_epi8(Rv3, Rs, cv3);
        Rs = _mm256_srli_epi32(Rv4, 16);
        Rv4 = _mm256_blendv_epi8(Rv4, Rs, cv4);

        // Cannot trivially replace the multiply as mulhi_epu32 doesn't exist (only
        // mullo). However we can use _mm256_mul_epu32 twice to get 64bit results
    ...
    for (z = NX - 1; z >= 0; z--) RansEncFlush(&ransN[z], &ptr);
...
static unsigned char *rans_uncompress_O0_32x16(unsigned char *in,
    ...
    if (!out) {
        out = static_cast<unsigned char *>(malloc(out_sz));
        *out_size = out_sz;
    }
    ...
    for (z = 0; z < NX; z++) RansDecInit(&R[z], &cp);
    ...
    for (i = 0; i < out_end; i += NX) {
        ...
        *(uint64_t *)&out[i + 8] = _mm256_extract_epi64(sv1, 2);
        ...
        Rv3 = _mm256_add_epi32(
            _mm256_mullo_epi32(_mm256_srli_epi32(Rv3, TF_SHIFT), fv3), bv3);
        Rv4 = _mm256_add_epi32(
            _mm256_mullo_epi32(_mm256_srli_epi32(Rv4, TF_SHIFT), fv4), bv4);
        ...
        renorm_mask3 = _mm256_cmpgt_epi32(
            _mm256_set1_epi32(RANS_BYTE_L - 0x80000000), renorm_mask3);
        renorm_mask4 = _mm256_cmpgt_epi32(
            _mm256_set1_epi32(RANS_BYTE_L - 0x80000000), renorm_mask4);
    ...
    for (z = out_sz & (NX - 1); z-- > 0;) out[out_end + z] = ssym[R[z] & mask];
...
#ifdef UNUSED
static void hist1_1(unsigned char *in, unsigned int in_size, int F0[256][256],
                    int T0[256]) {
...
static void hist1_4(unsigned char *in, unsigned int in_size, int F0[256][256],
                    int *T0) {
...
static unsigned char *rans_compress_O1_32x16(unsigned char *in, unsigned int in_size,
                                             unsigned char *out,
                                             unsigned int  *out_size) {
    unsigned char *cp, *out_end;
    ...
    for (z = 1; z < NX; z++) F[0][in[z * (in_size / NX)]]++;
    T[0] += NX - 1;
    ...
    *cp++ = 0;
    ...
    int isz4 = in_size / NX;
    int iN[NX];
    for (z = 0; z < NX; z++) iN[z] = (z + 1) * isz4 - 2;

    unsigned char lN[NX];
    for (z = 0; z < NX; z++) lN[z] = in[iN[z] + 1];
    ...
    for (; iN[0] >= 0;) {
        ...
        RansEncSymbol *sN[NX];
        for (z = 0; z < NX; z++) sN[z] = &syms[c[z] = in[iN[z]]][lN[z]];

#    define SET1x(a, b, x)                                                  \
        __m256i a##1 = _mm256_set_epi32(b[7]->x, b[6]->x, b[5]->x, b[4]->x, \
                                        b[3]->x, b[2]->x, b[1]->x, b[0]->x)
#    define SET2x(a, b, x)                                                      \
        __m256i a##2 = _mm256_set_epi32(b[15]->x, b[14]->x, b[13]->x, b[12]->x, \
                                        b[11]->x, b[10]->x, b[9]->x, b[8]->x)
#    define SET3x(a, b, x)                                                      \
        __m256i a##3 = _mm256_set_epi32(b[23]->x, b[22]->x, b[21]->x, b[20]->x, \
                                        b[19]->x, b[18]->x, b[17]->x, b[16]->x)
#    define SET4x(a, b, x)                                                      \
        __m256i a##4 = _mm256_set_epi32(b[31]->x, b[30]->x, b[29]->x, b[28]->x, \
                                        b[27]->x, b[26]->x, b[25]->x, b[24]->x)
#    define SETx(a, b, x) \
        SET1x(a, b, x);   \
        SET2x(a, b, x);   \
        SET3x(a, b, x);   \
        ...
        // rcp_shift[z]); ransN[z] = ransN[z] + bias[z] + q * cmpl_freq[z];
    ...
    for (z = NX - 1; z >= 0; z--) RansEncPutSymbol(&ransN[z], &ptr, &syms[0][lN[z]]);
...
static unsigned char *rans_uncompress_O1_32x16(unsigned char *in,
                                               unsigned int   in_size,
                                               unsigned char *out,
                                               unsigned int  *out_size) {
    /* Load in the static tables */
    ...
    int       z;
    uint8_t  *ptr = cp;
    for (z = 0; z < NX; z++) RansDecInit(&ransN[z], &ptr);

    int isz4 = out_sz / NX;
    int lN[NX] = {0}, iN[NX];
    for (z = 0; z < NX; z++) iN[z] = z * isz4;
    ...
    for (; iN[0] < isz4;) {
        ...
        __m256i Sv1 =
            _mm256_i32gather_epi32x((int *)&s3[0][0], masked1, sizeof(s3[0][0]));
        __m256i Sv2 =
            _mm256_i32gather_epi32x((int *)&s3[0][0], masked2, sizeof(s3[0][0]));
        ...
        __m256i Sv3 =
            _mm256_i32gather_epi32x((int *)&s3[0][0], masked3, sizeof(s3[0][0]));
        __m256i Sv4 =
            _mm256_i32gather_epi32x((int *)&s3[0][0], masked4, sizeof(s3[0][0]));
        ...
        Rv1 = _mm256_add_epi32(
            _mm256_mullo_epi32(_mm256_srli_epi32(Rv1, TF_SHIFT_O1), fv1), bv1);
        Rv2 = _mm256_add_epi32(
            _mm256_mullo_epi32(_mm256_srli_epi32(Rv2, TF_SHIFT_O1), fv2), bv2);
        ...
        //  R[z] = f[z] * (R[z] >> TF_SHIFT_O1) + b[z];
        Rv3 = _mm256_add_epi32(
            _mm256_mullo_epi32(_mm256_srli_epi32(Rv3, TF_SHIFT_O1), fv3), bv3);
        Rv4 = _mm256_add_epi32(
            _mm256_mullo_epi32(_mm256_srli_epi32(Rv4, TF_SHIFT_O1), fv4), bv4);
        ...
        if (++tidx == 32) {
            ...
            for (z = 0; z < NX; z++) {
                // replace by gathers?
                *(uint64_t *)&out[iN[z]] = ((uint64_t)(u.tbuf[0][z]) << 0) +
                                           ((uint64_t)(u.tbuf[1][z]) << 8) +
                                           ((uint64_t)(u.tbuf[2][z]) << 16) +
                                           ((uint64_t)(u.tbuf[3][z]) << 24) +
                                           ((uint64_t)(u.tbuf[4][z]) << 32) +
                                           ((uint64_t)(u.tbuf[5][z]) << 40) +
                                           ((uint64_t)(u.tbuf[6][z]) << 48) +
                                           ((uint64_t)(u.tbuf[7][z]) << 56);
                *(uint64_t *)&out[iN[z] + 8] = ((uint64_t)(u.tbuf[8 + 0][z]) << 0) +
                                               ((uint64_t)(u.tbuf[8 + 1][z]) << 8) +
                                               ((uint64_t)(u.tbuf[8 + 2][z]) << 16) +
                                               ((uint64_t)(u.tbuf[8 + 3][z]) << 24) +
                                               ((uint64_t)(u.tbuf[8 + 4][z]) << 32) +
                                               ((uint64_t)(u.tbuf[8 + 5][z]) << 40) +
                                               ((uint64_t)(u.tbuf[8 + 6][z]) << 48) +
                                               ((uint64_t)(u.tbuf[8 + 7][z]) << 56);
                *(uint64_t *)&out[iN[z] + 16] =
                    ((uint64_t)(u.tbuf[16 + 0][z]) << 0) +
                    ((uint64_t)(u.tbuf[16 + 1][z]) << 8) +
                    ((uint64_t)(u.tbuf[16 + 2][z]) << 16) +
                    ((uint64_t)(u.tbuf[16 + 3][z]) << 24) +
                    ((uint64_t)(u.tbuf[16 + 4][z]) << 32) +
                    ((uint64_t)(u.tbuf[16 + 5][z]) << 40) +
                    ((uint64_t)(u.tbuf[16 + 6][z]) << 48) +
                    ((uint64_t)(u.tbuf[16 + 7][z]) << 56);
                *(uint64_t *)&out[iN[z] + 24] =
                    ((uint64_t)(u.tbuf[24 + 0][z]) << 0) +
                    ((uint64_t)(u.tbuf[24 + 1][z]) << 8) +
                    ((uint64_t)(u.tbuf[24 + 2][z]) << 16) +
                    ((uint64_t)(u.tbuf[24 + 3][z]) << 24) +
                    ((uint64_t)(u.tbuf[24 + 4][z]) << 32) +
                    ((uint64_t)(u.tbuf[24 + 5][z]) << 40) +
                    ((uint64_t)(u.tbuf[24 + 6][z]) << 48) +
                    ((uint64_t)(u.tbuf[24 + 7][z]) << 56);
                iN[z] += 32;
            }
    ...
    if (1) {
        ...
        int T;
        for (z = 0; z < NX; z++)
            for (T = 0; T < tidx; T++) out[iN[z]++] = u.tbuf[T][z];
    }
 ...
 */
static unsigned char *rans_compress_to_32x16(unsigned char *in, unsigned int in_size,
                                             unsigned char *out,
                                             unsigned int *out_size, int order) {
    return order ? rans_compress_O1_32x16(in, in_size, out, out_size)
                 : rans_compress_O0_32x16(in, in_size, out, out_size);
}

static unsigned char *rans_compress_32x16(unsigned char *in, unsigned int in_size,
                                          unsigned int *out_size, int order) {
    return rans_compress_to_32x16(in, in_size, NULL, out_size, order);
}

static unsigned char *rans_uncompress_to_32x16(unsigned char *in,
                                               unsigned int   in_size,
                                               unsigned char *out,
                                               unsigned int *out_size, int order) {
    return order ? rans_uncompress_O1_32x16(in, in_size, out, out_size)
                 : rans_uncompress_O0_32x16(in, in_size, out, out_size);
}

static unsigned char *rans_uncompress_32x16(unsigned char *in, unsigned int in_size,
                                            unsigned int *out_size, int order) {
...
int main(int argc, char **argv) {
    ...
    while ((opt = getopt(argc, argv, "o:dt")) != -1) {
        ...
        fprintf(
            stderr, "Took %ld microseconds, %5.1f MB/s\n",
...
#define COMPRESSION_BLOCK_SIZE_BYTES (1 * 1024 * 1023)
...
static inline void compress(const std::vector<B> &src, std::vector<std::byte> &dest,
    ...
    size_t total_bytes = src.size() * sizeof(B);
    size_t offset = 0;

    while (offset < total_bytes) {
        unsigned int current_block_size = static_cast<unsigned int>(
            std::min<size_t>(COMPRESSION_BLOCK_SIZE_BYTES, total_bytes - offset));

        unsigned int out_size = 0;
        std::byte   *ptr = reinterpret_cast<std::byte *>(
            rans_compress_32x16(const_cast<unsigned char *>(src_ptr + offset),
                                  current_block_size, &out_size, order));

        size_t current_dest_size = dest.size();
        dest.resize(current_dest_size + sizeof(unsigned int) + out_size);

        std::memcpy(dest.data() + current_dest_size, &out_size,
                    sizeof(unsigned int));
        std::memcpy(dest.data() + current_dest_size + sizeof(unsigned int), ptr,
                    out_size);
...
static inline void decompress(const std::byte *from, uint32_t bytes, std::byte *to,
    ...
    while (src_offset < bytes && dest_offset < uncompressed_bytes) {
        ...
        dest_offset += expected_uncomp_size;
    }
}
}  // namespace rans_static
